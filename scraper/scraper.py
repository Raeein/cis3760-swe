...
def get_job_cards_from_html(html_string: str, job_board_name: str) -> list[str]:
    job_cards = []
...
def get_job_info(job_title: str, location: str, specified_job_boards: list[str] = []) -> list[dict]:
    ...
    for job_board_name in job_board_search_list:
        ...
        for job_card in job_cards:
            job_json_object = {}
            job_url = job_board_list_object[job_board_name]["job_board_base_url"] + job_card.find('a', href=True)['href']
            ...
            if job_url is not None:
                ...
                job_json_object["title"] = get_job_attribute(driver.page_source, "job_title", "Indeed")
                job_json_object["company"] = get_job_attribute(driver.page_source, "job_company", "Indeed")
                job_json_object["location"] = get_job_attribute(driver.page_source, "job_location", "Indeed")
                job_json_object["employment_type"] = get_job_attribute(driver.page_source, "job_employment", "Indeed")
                job_json_object["salary"] = get_job_attribute(driver.page_source, "job_pay", "Indeed")
                job_json_object["description"] = get_job_attribute(driver.page_source, "job_description", "Indeed")
...
if __name__ == "__main__":
    get_job_info("Software Developer", "Toronto, On")
